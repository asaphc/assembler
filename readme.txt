...
III. protect_value: this is optinal - if the user chooses to implement this function, when the user
     		    askes for the data, it will recive a copy created by protect_value, and thus will
		    not be able to change the memory of the structure itself.
...
c. label handler - deals with all aspects to do with labels - defined labels, entrys and externs
   	 	   checks the validity and stores the labels. also used to update labels, entrys
		  externs.
...
a. cmd_table.data
this file includes all the information about the commands of the assembly language
...
b. reserved.data
a list of reserved word of the assembly language. used for validating labels.
